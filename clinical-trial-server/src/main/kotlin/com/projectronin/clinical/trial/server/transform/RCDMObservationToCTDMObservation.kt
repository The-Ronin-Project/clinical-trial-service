...
class RCDMObservationToCTDMObservation(
    ...
    fun rcdmObservationToCTDMObservation(fhirPatientId: String, rcdmObservation: Observation): Observation? {
        ...
        if (rcdmObservation.subject?.isForType("Patient") == true) {
            ...
            val dataDictionaryRow = dataDictionaryService.getDataDictionaryByCode(obsCodeSystem, obsCodeValue) ?: throw Exception("Observation code $obsCodeValue not in data dictionary for CTDM")
            val ctdmSubject = subjectDAO.getSubjectByFhirId(fhirPatientId) ?: throw Exception("No subject found for Ronin FHIR Id: $fhirPatientId")
